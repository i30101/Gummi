      ...
      <div className="flex gap-4 overflow-x-auto hide-scrollbar">
        {highlights.map((highlight, index) => (
          ...
          >
            ...
            <span className="text-[11px] text-(--text-secondary) font-medium truncate w-16 text-center">
              {highlight.label}
            </span>
          </button>
        ))}
        ...
        <button className="flex flex-col items-center gap-1.5 shrink-0 group">
          <div className="relative w-16 h-16 md:w-[72px] md:h-[72px] group-hover:scale-105 transition-transform">
            <div className="w-full h-full rounded-full border-2 border-dashed border-(--border) flex items-center justify-center group-hover:border-(--text-tertiary) transition-colors">
          ...
          <span className="text-[11px] text-(--text-tertiary) font-medium">New</span>
        </button>
      </div>
    </div>
  );
